...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        currentProducts = (allProducts || []).filter(p => (p.stock ?? p.quantidade ?? p.quantity ?? 1) > 0);
    } catch (error) {
    ...
    if (catalogSection) {
        catalogSection.style.display = currentProducts.length > 0 ? '' : 'none';
    }
    ...
    // Brand nav links → fill search + scroll to catalog
    document.querySelectorAll('[data-brand-nav]').forEach(link => {
        link.addEventListener('click', (e) => {
            e.preventDefault();
            const brand = link.dataset.brandNav;
            const searchInput = document.getElementById('search-input');
            if (searchInput) {
